...
const releaseRoot = path.resolve("public/releases/art-expansion-batch-05-1.9.0");
const baseUrl = new URL("/Museum-Codex/releases/art-expansion-batch-05-1.9.0/", window.location.href).href;

const releaseFetcher = vi.fn<typeof fetch>((input: RequestInfo | URL) => {
  const url = new URL(typeof input === "string" || input instanceof URL ? input.toString() : input.url);
  const marker = "/releases/art-expansion-batch-05-1.9.0/";
...
describe("MUSEUM-09D-WAVE-01 formal browser release", () => {
  ...
  it("loads initial, catalog, and interaction closures from the physical bundle", async () => {
    ...
    expect(release.release.artists).toHaveLength(258);
    ...
    expect(catalog.data.artworks).toHaveLength(2471);
  ...
  it("loads every expanded local-search shard", async () => {
    ...
    expect(records).toHaveLength(4039);
    expect(records.some((record) => record.stable_id === "artist:m09a-aic_api-13343")).toBe(true);
  });
});
