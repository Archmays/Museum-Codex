...
const releaseRoot = path.resolve("public/releases/art-expansion-batch-02-1.6.0");
const baseUrl = new URL("/Museum-Codex/releases/art-expansion-batch-02-1.6.0/", window.location.href).href;

const releaseFetcher = vi.fn<typeof fetch>((input: RequestInfo | URL) => {
  const url = new URL(typeof input === "string" || input instanceof URL ? input.toString() : input.url);
  const marker = "/releases/art-expansion-batch-02-1.6.0/";
...
describe("MUSEUM-09C formal browser release", () => {
  ...
  it("loads initial, catalog, and interaction closures from the physical bundle", async () => {
    ...
    expect(release.release.artists).toHaveLength(111);
    ...
    expect(catalog.data.artworks).toHaveLength(1017);
  ...
  it("loads every expanded local-search shard", async () => {
    ...
    expect(records).toHaveLength(1745);
