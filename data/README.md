...
MUSEUM-03B reviewed art data lives under `reviewed/art/museum-03b/<version>/`. Its sealed `package-v1/` is an internal, canonical-JSON evidence package: the package manifest closes exact files, bytes, SHA-256 values, schemas, IDs, references, decision inputs, rights records, and graph input. It deliberately contains no media bytes, no `published` state, and no public dataset release; the Pages build does not consume this directory.
