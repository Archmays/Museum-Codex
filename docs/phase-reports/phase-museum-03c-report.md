...
流水线默认离线；真实媒体获取必须同时提供 `--live --download-media`。下载器使用 direct `http.client` HTTPS，忽略环境代理；每一跳验证 trusted host、公网 DNS、TLS hostname 和实际 peer IP，拒绝 Cookie/Auth、私网、未知重定向、HTML、MIME/magic 不一致、超限内容和覆盖写入。每来源并发为 1，AIC/Commons 至少 1 秒间隔；支持 Retry-After、ETag/304、lock、临时文件、fsync、atomic no-overwrite、hash dedupe 和 expected-hash 幂等恢复。
...
全仓一次性 `pytest -q` 曾两次触及 20/30 分钟外部执行上限，未产生失败堆栈。分组诊断确认主要额外耗时来自尚待 Stage C 重构的旧 M04 zero-media fixture matrix：它对 20 个候选 release 变体重复执行约 300 个 record schema 校验。该旧 M04 门禁不作为 M03C 通过证据，也没有被删除或放宽；将在 MUSEUM-04-REVISED 正式媒体 release 中替换并完成最终全仓回归。
