...
- Twenty-seven synthetic changed-path cases cover docs-only, phase-local, shared schema/loader/rights, dependencies, workflow, public bytes, route suites, multi-commit, delete, rename, empty before SHA, mixed changes, and manual targeted/full modes.
...
- Local full gate count is zero. GitHub ran the clean final-full validation exactly once to success, and Pages created exactly one M08 runtime deployment. A later docs/verification-only repair used the affected closure and did not rebuild or deploy.
...
- Final controlled p95 is 23.2662 ms for the current index and 52.9409 ms for 1,000 fixed synthetic records. The route is 87,894 gzip bytes and the index is 45,663 gzip bytes.
...
- Home is 101,981 gzip bytes (+1.921% from M07); largest non-map route 87,894; map route 533,958; low-bandwidth initial transfer p95 151,339. Desktop/mobile first-interactive p95 are 167.9666/439.0007 ms, interaction p95 17.7 ms, CLS 0, external request/media preload/geolocation counts 0.
...
## G — Scope, decisions, rights/source, and publication: pass
...
- GitHub Actions final-full job passed 474 Python tests, 98 Vitest tests, 39 Playwright cases, lint/typecheck, all release/rights/security validators, the production build, and sealed performance gates. Pages deployment `5508931387` succeeded for runtime commit `b36ac365b13ca24afa2d89f6dac6b680036a04af`.
- The parent run `29677859518` concluded `failure` only after deployment because the post-deploy verifier imported a package absent from that runner. Commit `7e0210765651dd3026ea93f15caa7964f2b10563` made the verifier pure-standard-library; the bounded recovery then closed 316/316 manifest files and 41,848,680/41,848,680 bytes with the exact manifest/content hashes. Subsequent Actions run `29678982443` passed its affected closure in 42 seconds with no rebuild or deployment.
...
| P2 | A search benchmark run shared CPU with the 203 MB ledger hash closure and exceeded p95. | Kept the threshold unchanged and reran the benchmark in isolation; final current/1k p95 closed at 23.2662/52.9409 ms. |
| P2 | The successful deployment run's post-deploy verifier imported `referencing`, which was absent on the minimal deploy runner. | Rewrote only the verifier closure to Python standard-library validation, preserved all byte/hash checks, completed the online closure without redeploying, and proved the repair in a 42-second scoped Actions run. |
