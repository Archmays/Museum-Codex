...
这些事项没有在 MUSEUM-00 被静默决定。当前共 **10** 项。
...
| OD-003 | GitHub 仓库与 Pages 的最终公开可见性 | 启用 Pages 前 | 仓库按同类项目私有；不启用 Pages | 用户 |
| OD-004 | 首批 12 位艺术家组合 | MUSEUM-03 | 不确定名单、不采集正式批次 | 用户 + curator |
...
| OD-007 | 高分图像以来源 IIIF、自托管衍生物或混合方案的比例 | MUSEUM-03/05 | 逐资产决定，不复制未知字节 | rights + engineering |
